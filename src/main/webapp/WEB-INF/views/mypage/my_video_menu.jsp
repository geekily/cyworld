...
<script type="text/javascript">
	...
	function VideosOfThisMenu(folderName){
		...
		iframeVideo.src="video/my_r_video_page.action?folderName="+encodedFolderName;
	...
	function deleteFolder(folderName){
		...
		var url = "<%=cp%>/cy/video/deleteOneFolder.action";
		...
		if(folderName==currentFolderName.value){
			 ...
			 $.post(url,{folderName:value1,currentFolderName:value2},function(args){
				 location.replace('my_video.action?mode=deleteMode');
			  });
	...
	$(document).ready(function(){
		 $("#addMenu").click(function(){
			 ...
			 var params = "folderName=" + $("#newFolderNameId").val()+ "&currentFolderName=" + $("#currentFolderName").val()
			 	+ "&privacy=" + $("#privacy").val();
			 
			 $.ajax({
				 type:"POST",
				 url:"<%=cp%>/cy/video/createMenu_ok.action",
				 data:params,
		 ...
		 $("#deleteMenu").click(function(){
			 ...
			 $.ajax({
				 type:"POST",
				 url:"<%=cp%>/cy/video/showDeleteMenuButton.action",
				 data:params,
				 success:function(args){
					 	$("#menuData").html(args);
		...
		$("#stopDeleteMenu").click(function(){
			 ...
			 $.ajax({
				 type:"POST",
				 url:"<%=cp%>/cy/video/menuList.action",
				 data:params,
				 success:function(args){
					 	$("#menuData").html(args);
	...
	function checkCreateMenu(){
		...
		if(!newFolderNameId.value.trim()){
			swal("앨범 이름을 입력하세요.", "", "<%=cp %>/resources/images/cy_logo.png");
			return true;
		}
	...
	function checkDeleteMenu(){
			...
			if(totalFolderData==0){
				swal("삭제할 앨범이 없습니다.", "", "<%=cp %>/resources/images/cy_logo.png");
				return true;
			}
	...
	function sendEditedFolderName(index, originalFolderNameToBeEdited){
		...
		var url = "<%=cp%>/cy/video/editFolderName.action";
		
		if(!editedFolderName){
			swal("수정할 앨범 이름을 입력하세요.", "", "<%=cp %>/resources/images/cy_logo.png");
			return
		}
...
<form name="main">
		...
		<c:set var="index" value="${status.index }"/>
				...
				<div id="editFolderNameTextBar${index }" style="display: none;">
					<div style="float: left;">
						...
						<input type="text" id="editedFolderName${index }" Placeholder="${dto.folderName }" size="13" maxlength="10" onkeydown="characterCheckk(${index})" style="font-size: 7pt;padding-left: 4px;">
					</div>
					...
					<div align="right" style="float: right;">
						<font style="color: #1294AB;font-weight: bold;">
							<span style="font-size:8pt;"
							onmouseover="this.style.color='#FF5E00';" onmouseout="this.style.color='#1294AB';"
				...
				<c:if test="${dto.folderName!=folderName }">
					...
					<div style="float: left;">
						...
						<input type="text" id="editedFolderName${index }" Placeholder="${dto.folderName }" size="13" maxlength="10" style="font-size: 7pt;padding-left: 4px;">
					</div>
					<div style="border: 0px solid black;float: left;width: ">
						<c:if test="${dto.privacy==2 }">
	...
	<div id="newFolder1" style="border: 0px solid black;width: 150px;height: 30px;text-align: left;border: none;">
		...
		<div id="newFolder" style="border: 0px solid black;">
			<font  style="font-size:8pt;color: #1294AB;font-weight: bold;">
				<span  onmouseover="this.style.color='#FF5E00';" onmouseout="this.style.color='#1294AB';"
				onclick="javascript:openNewFolder()">새 앨범 추가</span>
			</font>
		</div>
		<div id="deleteMenu" style="border: 0px solid black;">
			<font  style="font-size:8pt;color: #1294AB;font-weight: bold;">
				<span  onmouseover="this.style.color='#FF5E00';" onmouseout="this.style.color='#1294AB';">앨범 삭제</span>
			</font>
		</div>
		</c:if>
	</div>
	...
	<c:if test="${mode=='deleteMode' }">
		<div id="stopDeleteMenu" style="border: 0px solid black;text-align: left;padding-left: 13px;">
			<font  style="font-size:8pt;color: #1294AB;font-weight: bold;">
				<span onmouseover="this.style.color='#FF5E00';" onmouseout="this.style.color='#1294AB';" >앨범 삭제 취소</span>
			</font>
		</div>
	</c:if>
						
							<div id="newFolder2" style="border: 1px solid black;width: 150px;height: 30px;display: none;border: none;" align="left">
								<div style="float: left;">
									<font  style="font-size:8pt;color: #1294AB;font-weight: bold;">앨범 이름</font><br/>
								</div>
