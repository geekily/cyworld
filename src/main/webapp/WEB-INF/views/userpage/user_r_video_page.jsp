...
<script type="text/javascript">
	...
	$(document).ready(function(){
		 $("#sendButton0").click(function(){
			 ...
			 $.ajax({
				 type:"POST",
				 url:"<%=cp%>/cy/user/video/insertReple.action",
				 data:params,
	...
	function listPage(page,value1,value2,value3){
	 ...
	 var url = "<%=cp%>/cy/user/video/listReple.action";
	...
	function deleteOneRepleData(value1,value2,value3,value4){
		...
		var url = "<%=cp%>/cy/user/video/deleteOneRepleData.action";
	...
	function repleAfterScrap(index) {
		var params = "replierId=${sessionScope.session.userId }&content=퍼가요~♡&userId=" + $("#userId"+index).val()
		 + "&videoNum=" + $("#videoNum"+index).val()+ "&replierName=${sessionScope.session.userName }&index="+index
			...
			<td align="right" style="padding-right: 10px;padding-bottom: 3px;">
				<font>
					<c:if test="${cyMemberDTO!=null }">
						...
						onclick="window.open('scrapVideo.action?owner=${dto.owner}&originalVideoNum=${dto.num }&index=${index }','window_moving','width=300,height=300,left=300,top=300,location=no,status=no,scrollbars=no');" target="_blank">
							스크랩
						</span>
					</c:if>
					<c:if test="${cyMemberDTO==null }">
